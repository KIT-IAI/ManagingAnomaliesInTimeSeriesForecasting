...
from pywatts.modules import ClockShift, CalendarExtraction, CalendarFeature, Sampler
from pywatts.modules.wrappers import FunctionModule, SKLearnWrapper


def forecast(hparams):
    """
    Calculates a forecast using a Linear Regression
    """
    pipeline = Pipeline(name='svr', path=os.path.join('run', hparams.name, 'forecasting', 'lr'))
