...
def last_week_forecast(hparams, y):
    """
    Calculates last week's value
    """
...
def forecast(hparams):
    """
    Calculates a forecast using last week's value
    """
    pipeline = Pipeline(name='last_week', path=os.path.join('run', hparams.name, 'forecasting', 'last_week'))

    #####
    # Apply forecasting model
    ###
    y_hat = FunctionModule(lambda y: last_week_forecast(hparams, y), name='forecast')(y=pipeline['y_target'])
