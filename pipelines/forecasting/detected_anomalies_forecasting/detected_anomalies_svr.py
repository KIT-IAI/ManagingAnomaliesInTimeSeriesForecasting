...
from pywatts.modules import ClockShift, Sampler, CalendarExtraction, CalendarFeature
from pywatts.modules.wrappers import FunctionModule, SKLearnWrapper


def forecast(hparams):
    """
    Calculates a forecast using a SVR that considers information on detected anomalies
    """
    pipeline = Pipeline(name='detected_anomalies_svr', path=os.path.join('run', hparams.name, 'forecasting',
                                                                         'detected_anomalies_svr'))
    ...
    forecast = SKLearnWrapper(
        module=MultiOutputRegressor(LinearSVR(**new_par), n_jobs=2),
        name='forecast'
    )(
        **load_history,
        **anomalies_history,
